...
class MoveTest < Minitest::Unit::TestCase
  ...
  def test_not_fall_from_bottom_edge
    ...
    @p.parse 'MOVE'
    lambda { @p.parse 'REPORT' }.must_output "2,0,SOUTH\n"
  end

  def test_not_fall_from_left_edge
    @p.parse 'PLACE 0,2,WEST'
    @p.parse 'MOVE'
    lambda { @p.parse 'REPORT' }.must_output "0,2,WEST\n"
  end

  def test_not_fall_from_right_edge
    @p.parse 'PLACE 4,2,EAST'
    @p.parse 'MOVE'
    lambda { @p.parse 'REPORT' }.must_output "4,2,EAST\n"
  end
  ...
  def test_walk_in_a_circle_on_edge_anticlockwise
    ...
    @p.parse 'MOVE'
    @p.parse 'MOVE'
    lambda { @p.parse 'REPORT' }.must_output "0,0,SOUTH\n"
  end
